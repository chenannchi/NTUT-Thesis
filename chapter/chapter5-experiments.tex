...
為降低實驗結果受到非研究變因影響，本研究於消融實驗中使用相同的任務集合、基礎模型、評估指標與主要執行設定，並僅改變原子化經驗與動態局部修復兩項機制是否啟用，以比較各機制對任務完成率的影響。然而，大型語言模型仍具有生成不確定性，即使在相同設定下，單次執行結果仍可能受到模型輸出差異、API 呼叫順序或中間推理結果影響。因此，本研究結果可反映各設定在相同控制條件下的相對表現，但仍可能存在隨機生成造成的估計誤差。
